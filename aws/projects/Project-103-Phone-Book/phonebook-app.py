...
app.config['MYSQL_DATABASE_DB'] = 'demir_phonebook'
...
# Write a function named `init_todo_db` create phonebook table within demir_phonebook db, if it doesn't exist

def init_phonebook_db():
    phonebook_table = """
    CREATE TABLE IF NOT EXISTS demir_phonebook.phonebook(
...
def find_records():
    if request.method == 'POST':
        ...
        return render_template('index.html', persons_html=persons_app, keyword=keyword, show_result=True, developer_name='Captain_Demir')
    else:
        return render_template('index.html', show_result=False, developer_name='Captain_Demir')
